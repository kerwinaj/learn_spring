 ...
 * 然后会分别设置初始化值, 即赋值为 3, new Price(2.8)和20;
 ...
 *
 * 按代码先后, 执行静态变量定义 和 静态代码块;
 * 按代码先后, 为两个类变量(INSTANCE和INIT_PRICE)分配内存空间, 这时候值为默认值(null和0.0), 然后会分别设置初始化值, 即赋值为 new Price(2.8) 和 20;
 * 在赋值为 new Price(2.8) 的过程中, 按代码先后, 执行初始化的方法(代码块和实例变量定义, 构造方法最后);
 */
